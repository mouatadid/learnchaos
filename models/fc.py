...
def fc(job_id, data_dir, j='all', weighted=True, num_epochs=10, pretrained=False, 
...
#        model.add(Flatten(input_shape = (20, 20, 1)))
    ...
    #callbacks
    path_log = 'experiments\\results_fc_'+str(num_layers)+'\\N'+str(N)+'_fc_'+str(num_layers)+'_checkpoints_'+str(test_mode)+'_'+str(l96_var)+'_'
    ...
    csv_logger = CSVLogger(path_log+'training_fc_'+str(num_layers)+'.log', separator=',', append=True)
    
    
    if pretrained ==True:
        path_weights = 'experiments\\results_fc_'+str(num_layers)+'\\'+best_job_id+'.hdf5'#+str(data_dir[17:-1])+str(best_job_id)+'\\'
...
#        weights_id = str(weights_id[0])
    ...
    model_params = {'model_name'   : 'fc_'+str(num_layers),
                    'job_id'       : job_id,
