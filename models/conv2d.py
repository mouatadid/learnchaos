...
from keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Cropping2D
import numpy as np
...
def conv2d(job_id, data_dir, j='all', weighted=True, num_epochs=10, pretrained=False, 
    ...
    if l96_var != 'XY':
        model.add(Cropping2D(cropping=((0, 0), (left_crop, right_crop)),
                         input_shape=(20, 20, 1))) 
        #tuple of 2 tuples of 2 ints: ((top_crop, bottom_crop), (left_crop, right_crop))
    ...
    #callbacks
    path_log = 'experiments\\results_conv2d_'+str(num_layers)+'\\N'+str(N)+'_conv2d_'+str(num_layers)+'_checkpoints_'+str(test_mode)+'_'+str(l96_var)+'_'
    ...
    csv_logger = CSVLogger(path_log+'training_conv2d_'+str(num_layers)+'.log', separator=',', append=True)
    
    
    if pretrained ==True:
        path_weights = 'experiments\\results_conv2d_'+str(num_layers)+'\\N'+str(N)+'_conv2d_'+str(num_layers)+'_checkpoints_'+str(best_job_id)+'.hdf5'#+str(data_dir[17:-1])+str(best_job_id)+'\\'
...
#        weights_id = str(weights_id[0])
    ...
    model_params = {'model_name'   : 'conv2D',
                    'job_id'       : job_id,
