# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 16:51:35 2019
...
def lr(job_id, data_dir, j='all', weighted=True, num_epochs=10, pretrained=False, 
    ...
    #callbacks
    path_log = 'experiments\\results_lr_'+str(num_layers)+'\\N'+str(N)+'_lr_'+str(num_layers)+'_checkpoints_'+str(test_mode)+'_'+str(l96_var)+'_'
    ...
    csv_logger = CSVLogger(path_log+'training_lr_'+str(num_layers)+'.log', separator=',', append=True)
    
    
    if pretrained ==True:
        path_weights = 'experiments\\results_lr\\N'+str(N)+'_lr_'+str(num_layers)+'_checkpoints_'+str(best_job_id)+'.hdf5'#+str(data_dir[17:-1])+str(best_job_id)+'\\'
...
#        weights_id = str(weights_id[0])
    ...
    model_params = {'model_name'   : 'lr_'+str(num_layers),
                    'job_id'       : job_id,
